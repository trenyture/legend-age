				...
				<table>
					...
					<tbody>
						<tr v-for="(line, index) in basketLines">
							...
							<td>{{ line.quantity }} {{ line.byFour ? `offre${line.quantity > 1 ? 's' : ''} promotionnelle de 2 exemplaires` : `exemplaire${line.quantity > 1 ? 's' : ''}` }}</td>
							<td>{{ parseFloat((line.byFour ? 35 : 29)/1.2).toFixed(2) }} €</td>
							<td>{{ parseFloat((line.quantity * (line.byFour ? 35 : 29) / 1.2)).toFixed(2) }} €</td>
							<td>
